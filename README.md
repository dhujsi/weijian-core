...
- 笔记与提醒（内置插件：`builtin_notes_reminders`）
- WebUI 管理页面（中文）
- 插件热重载（单插件/全量）
...
- 提醒（由内置插件提供）：
	...
	- `取消提醒 <id>`
	- `清空已完成提醒`
- 调度器：插件可注册后台调度循环（内置插件默认每 10 秒扫描 `pending` 提醒并触发）
...
- 页面：
	...
	- `GET /ui/logs`（运行日志）
	- `GET /ui/ext/notes`（由 `builtin_notes_reminders` 插件提供）
	- `GET /ui/ext/reminders`（由 `builtin_notes_reminders` 插件提供）
